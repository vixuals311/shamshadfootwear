...
BEGIN
    ...
    WITH role_defaults AS (
        SELECT p.key, 
            CASE _role
                WHEN 'manager' THEN p.key IN ('dashboard','inventory','price_check','clients','invoices','payments','cheques','recovery','city_recovery','returns','reports','audit_logs','settings')
