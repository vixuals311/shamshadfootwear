...
BEGIN
    ...
    IF _role = 'admin' THEN
        RETURN QUERY SELECT p.key, true FROM (VALUES 
            ...
            ('users'),('audit_logs'),('settings'),('bulk_clients')
        ) AS p(key);
        RETURN;
    END IF;
    ...
    WITH role_defaults AS (
        SELECT p.key, 
            CASE _role
                WHEN 'manager' THEN p.key IN ('dashboard','inventory','price_check','clients','invoices','payments','cheques','recovery','city_recovery','returns','reports','audit_logs','settings','bulk_clients')
                WHEN 'biller' THEN p.key IN ('dashboard','inventory','price_check','clients','invoices','returns')
                WHEN 'cashier' THEN p.key IN ('dashboard','payments','cheques','recovery','city_recovery')
                WHEN 'biller_cashier' THEN p.key IN ('dashboard','inventory','price_check','clients','invoices','payments','cheques','recovery','city_recovery','returns')
                ELSE false
            END AS default_access
        FROM (VALUES 
            ('dashboard'),('inventory'),('price_check'),('clients'),('invoices'),
            ('payments'),('cheques'),('recovery'),('city_recovery'),('returns'),('reports'),
            ('audit_logs'),('settings'),('bulk_clients')
        ) AS p(key)
    ),
    overrides AS (
